...
export async function generateStaticParams() {
  ...
  const slugRoutes = table.map((item: any) => item.slug);
...
export default async function BlogPost({
  ...
  const table = await getBlogTable<any>(config.notionProjectTableId);
  const publishedProjects = table.filter((p: any) => p.published);

  const post = table.find((t: any) => t.slug === params.projectSlug);
  const postIndex = publishedProjects.findIndex(
    (t: any) => t.slug === params.projectSlug
  );
